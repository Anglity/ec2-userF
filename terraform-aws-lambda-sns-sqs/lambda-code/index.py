...
def lambda_handler(event, context):
    ...
    for record in event.get('Records', []):
        try:
            if not record.get('body'):  # Verificar si el mensaje está vacío
                print("Mensaje vacío recibido, ignorando.")
                continue
            
            body = json.loads(record['body'])  # Intentar parsear el JSON
            
            # Si el mensaje proviene de SNS, el JSON viene dentro de "Message"
            if "Message" in body:
